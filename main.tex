...
Following the directional expectations developed earlier, we constructed the Policy-Value Orientation Composite (PVOC) as a standardized sum of the five dimensions, with higher values indicating an orientation profile expected, ex ante, to produce larger predicted burden effects. Because the composite is standardized within the study sample and the battery is newly constructed, PVOC is a theory-specified diagnostic aggregate rather than a validated psychometric scale. We treat the PVOC result as one diagnostic input among three rather than as the central test of value sensitivity, consistent with prior evidence that aggregate composite scores from constrained-choice batteries produce unstable estimates in small model samples. The PVOC serves as the primary independent variable in the orientation-effect models. Exploratory internal-consistency diagnostics and weighting-sensitivity checks are reported in the Online Supplement.
...
Table~\ref{tab:orientation_profiles} reports each model's measured policy-value orientation profile in the expanded fifteen-model sample across the five dimensions (autonomy protection, state trust, collective obligation, administrative-burden sensitivity, and coercion aversion), each measured on a 1--7 scale. Higher PVOC values indicate a profile expected, ex ante, to generate larger predicted burden effects (stronger autonomy protection, stronger burden sensitivity, stronger coercion aversion, weaker state trust, and weaker collective obligation). Because PVOC is standardized within the model sample, the values are relative to the expanded fifteen-model sample. The highest PVOC scores are observed for Llama 3.1 8B Instruct, Claude Haiku, and Llama 3.1 70B Instruct; the lowest for Mistral Large, GPT-4o, and DeepSeek-V3.2. The ordering does not map cleanly onto provider origin or access route, indicating that PVOC captures theoretically specified variation rather than a simple provider-level label.
