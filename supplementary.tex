...
\item[Collective responsibility:] \texttt{civic duty}, \texttt{protect others}, \texttt{communal benefit}, \texttt{vulnerable group}, \texttt{collective welfare}, \texttt{common good}, \texttt{herd immunity}, and phrases emphasizing community benefit. The earlier \texttt{public health} pattern was removed because it matched ``trust in public health (authorities)'' and ``public health center'' (the vaccination site), which express trust in authorities or location rather than collective responsibility.
...
Frames with little or no empirical variation are reported descriptively but excluded from the main frame-adjusted model. In the present analysis, autonomy infringement and procedural legitimacy each appeared in one of the 2,019 high-burden rationales (0.05\% each), and collective responsibility appeared in a single rationale (0.05\%) after the coding rule was corrected (the original bare ``public health'' pattern, which matched ``trust in public health'' and ``public health center,'' is removed). None of these three frames has usable variation. The main H2 model therefore retains access barriers and coercive backlash.
...
\begin{flushleft}
\footnotesize Notes: Pair-FE is the primary specification of the main text: one fixed effect per model--profile pair (405 pairs) plus the retained rationale-frame indicators, with standard errors clustered at the model--profile level. Collective responsibility is dropped from the pair-FE column because the corrected coding leaves only one positive rationale (0.05\%), giving the indicator no within-pair variation. Two-way FE replaces the pair indicators with separate model and profile fixed effects, absorbing additive heterogeneity only; its unadjusted \(p\)-values are 0.012 (access barriers) and 0.174 (coercive backlash), and its Benjamini--Hochberg adjusted values are 0.036 and 0.262. All \(p\)-values in the pair-FE column are Benjamini--Hochberg adjusted across the retained frames (\(q=0.10\)). \(N=2{,}019\) in both specifications. Positive coefficients indicate larger predicted burden effects.
\end{flushleft}
\end{table}
