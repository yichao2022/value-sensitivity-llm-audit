...
\subsection*{Leave-one-model-out under the secondary specification}
...
The pooled H3 estimates were re-estimated with one fixed effect per model--profile pair (108 pairs, twenty assigned-frame observations each) in place of the separate model and profile fixed effects, holding the clustering of standard errors at the model--profile level so that only the fixed-effect structure changes:
