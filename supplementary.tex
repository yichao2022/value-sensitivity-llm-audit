...
\title{Online Supplement: ``A Framework for Auditing Value Sensitivity in Large Language Model--Assisted Health Policy Simulation''}
\author{Yichao Jin}
\date{\today}
...
The manual inspection confirmed that pattern assignments reflected the codebook definitions rather than superficial keyword matches, and no systematic misclassification pattern was identified that would require pattern refinement. The coded prevalence estimates were therefore used as descriptive inputs to the frame--outcome association analysis.
...
We re-estimated each of the three main models twenty-seven times, excluding one synthetic profile at a time (27 fits per model). Coefficient ranges across the 27 fits are archived with the replication package. The H1 PVOC slope stays between $-2.26$ and $-2.07$ and is never statistically distinguishable from zero ($p$ between 0.26 and 0.30), so the H1 null is not produced by any single test case. For H2, the access-barrier coefficient remains positive in all 27 fits (range $+1.67$ to $+2.69$; $p<0.05$ in 25 of 27, reaching $p=0.097$ only when profile P18 or P27 is removed) and the coercive-backlash coefficient remains positive and significant in all 27 ($+6.61$ to $+8.16$, $p \le 0.0012$); the collective-responsibility coefficient stays positive but statistically indistinguishable from zero in every fit ($+0.32$ to $+1.04$, $p \ge 0.34$). For H3, all three frame contrasts retain both sign and significance in all 27 fits (autonomy $+4.64$ to $+5.18$; collective obligation $-11.01$ to $-10.70$; equity/access $+4.56$ to $+5.10$; $p<0.001$ throughout).
