...
\textit{SHA256 of the template string:} \texttt{1b98f920c7a2a1932b741233e800f99a62e1de84a1a172cd2038f1ad3c6970e4}
\end{quote}

The rendered user prompt therefore consists of the profile text and scenario text documented in this appendix, inserted into the template above. The two burden scenarios are fixed strings; their SHA256 hashes are \texttt{7307a8b8f499ebc0897c6fa0436fb8eafb3e77efeebf21ce91d8471989ec9b01} (low burden) and \texttt{c0bc058f951e4294842a4173f4cc2ac1e0475f1ff84d9438151cc9bd42967b52} (high burden). The 27 profile strings are the natural-language descriptions listed in Appendix~\ref{app:profiles}.
...
\midrule
Access barriers & 1.51 & 3.81 & Yes \\
Collective responsibility & -1.79 & 1.62 & No \\
Coercive backlash & 3.52 & 11.25 & Yes \\
PVC direct effect & -3.64 & -0.60 & Yes (negative) \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize Notes: Each row summarizes coefficients from fifteen leave-one-model-out specifications. The access-barrier and coercive-backlash coefficients are sign-stable across all specifications; collective responsibility is not. The direct PVC coefficient is negative in all fifteen specifications; it is reported for completeness as a covariate diagnostic, since H2 is specified as a frame--outcome association rather than a PVC effect.
\end{flushleft}
\end{table}
...
The frame-manipulation experiment used four system-prompt frames. Each frame is built from the same two fixed sentences: an opening sentence and a closing instruction block. The frame clause is inserted between them, immediately after the opening sentence. The neutral frame contains no frame clause, so the neutral system prompt is byte-identical to the baseline administrative-burden system prompt reported in Appendix~\ref{app:burden-prompts} (SHA256 \texttt{380d779e199673cdeaf808731aad01414b60e71edba7c23a57602241ff6dcffe}); the frame contrast is therefore measured against the true baseline condition. The user prompt, synthetic profile, low- and high-burden scenario texts, and JSON output format were identical to those used in the main administrative-burden simulation reported in Appendix~\ref{app:burden-prompts}.
...
\begin{itemize}
    \item \textbf{Neutral frame:} no clause inserted. Full system prompt SHA256: \texttt{380d779e199673cdeaf808731aad01414b60e71edba7c23a57602241ff6dcffe}

    \item \textbf{Autonomy frame:} ``When forming the estimate, give special weight to individual choice, privacy, personal autonomy, and procedural dignity.'' Full system prompt SHA256: \texttt{8b7d948eeee661603aa9898815ee3a01a058f43d4cc878cbc077e6cb5a5df243}

    \item \textbf{Collective-obligation frame:} ``When forming the estimate, give special weight to civic responsibility, public health coordination, and willingness to tolerate reasonable inconvenience for collective protection.'' Full system prompt SHA256: \texttt{fe7512cde053405edf59cbaaf6d1402f68d69e7c478984eee3d0fbc3a49a5ecb}

    \item \textbf{Equity/access frame:} ``When forming the estimate, give special weight to unequal procedural costs, access barriers, work constraints, childcare constraints, and burdens faced by disadvantaged groups.'' Full system prompt SHA256: \texttt{cba0156ababefd86611eb267ebf9ccddb1d60fe5a8cecf4e289871006042844a}
\end{itemize}
...
Mistral Large is a particularly informative robustness case. It has the lowest PVC score in the expanded fifteen-model sample, and all three frame manipulations shift its simulated burden effects in the expected directions. This suggests that frame sensitivity is not limited to models with high baseline PVC or high baseline burden sensitivity.
...
\end{tabular}
\begin{flushleft}
\footnotesize Notes: Each row reports the mean profile--repetition-level burden effect, \(\Delta = W^{low}-W^{high}\), for one model under one assigned frame. $N=135$ for each model--frame cell, corresponding to 27 profiles and five repetitions, except where a response failed the structured-output checks: Llama 3.1 70B Instruct equity/access ($N=134$), and Qwen3.7 Plus ($N=114$, 104, 108, and 120 across the neutral, autonomy, collective-obligation, and equity/access frames respectively, reflecting calls rejected by the serving endpoint). Positive values indicate larger predicted reductions in willingness under the high-burden condition.
\end{flushleft}
\end{table}
...
Table~\ref{tab:h1_expanded_robustness} summarizes the key H1 robustness results for the original nine-model sample and the expanded fifteen-model sample. The composite PVC index does not predict model-level burden effects in either sample. In the original nine-model sample, the PVC coefficient is negative and statistically indistinguishable from zero (\(\hat{\beta}=$-$4.24\), \(p=0.219\), \(R^2=0.206\)). After expanding to fifteen endpoints, the PVC coefficient remains weak and statistically indistinguishable from a null effect at conventional power (\(\hat{\beta}=$-$2.14\), \(R^2=0.090\)).

The expanded sample also changes the interpretation of the dimension-specific collective-obligation result. In the original nine-model sample, collective obligation (CO) was positively associated with model-level burden effects but did not reach conventional significance (\(\hat{\beta}=10.81\), \(p=0.179\), \(R^2=0.242\)). After the six OpenRouter-accessed models are added, the association shrinks and remains non-significant (\(\hat{\beta}=1.28\), \(p=0.725\), \(R^2=0.010\)). This indicates that the CO association is sensitive to model composition and should be interpreted as an exploratory signal at best rather than as a stable dimension-level predictor.

The expanded-sample measurement diagnostics also suggest caution in interpreting the composite PVC formula. CO is the only dimension with acceptable exploratory internal consistency, but removing CO from the composite does not change the substantive conclusion: the leave-CO-out PVC coefficient remains statistically indistinguishable from zero (Table~\ref{tab:h1_expanded_robustness}). This does not validate an alternative index; the composite association is weak under either weighting.
...
\midrule
PVC composite, \(N=9\) & $-$4.24 & 0.219 & 0.206 & Null \\
PVC composite, \(N=15\) & $-$2.14 & 0.278 & 0.090 & Null \\
Collective obligation, \(N=9\) & 10.81 & 0.179 & 0.242 & Not significant \\
Collective obligation, \(N=15\) & 1.28 & 0.725 & 0.010 & Not stable \\
Leave-CO-out PVC, \(N=15\) & $-$2.22 & 0.333 & 0.072 & Null \\
\bottomrule
\end{tabular}
\begin{flushleft}
