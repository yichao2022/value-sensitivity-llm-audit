...
PVOC direct effect & -3.64 & -0.60 & Yes (negative) \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize Notes: Each row summarizes coefficients from fifteen leave-one-model-out specifications. The access-barrier and coercive-backlash coefficients are sign-stable across all specifications; collective responsibility is not. The direct PVOC coefficient is negative in all fifteen specifications; it is reported for completeness as a covariate diagnostic, since H2 is specified as a frame--outcome association rather than a PVOC effect.
\end{flushleft}
\end{table}
